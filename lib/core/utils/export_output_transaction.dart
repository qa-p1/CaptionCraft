...
class ExportOutputTransaction {
  ...
  static Future<ExportOutputTransaction> create(
    String outputPath, {required Iterable<String> sourcePaths}
  ) async {
    ...
    if (type != FileSystemEntityType.notFound && type != FileSystemEntityType.file) {
      throw const FileSystemException('The export destination must be a regular file.');
    }
    for (final sourcePath in sourcePaths.where((path) => path.isNotEmpty)) {
      final source = File(sourcePath);
      final exists = await source.exists();
      final resolved = exists ? await source.resolveSymbolicLinks() : p.normalize(p.absolute(sourcePath));
      final equal = Platform.isWindows
          ? resolved.toLowerCase() == resolvedPath.toLowerCase()
          : p.equals(resolved, resolvedPath);
      if (equal || (type == FileSystemEntityType.file && exists &&
          await FileSystemEntity.identical(source.path, destination.path))) {
        throw const FileSystemException('Choose a destination different from your source media.');
      }
    }
    final original = type == FileSystemEntityType.file ? await destination.stat() : null;
    final directory = await destination.parent.createTemp('.captioncraft-export-');
  ...
  Future<void> commit() async {
    ...
    final currentType = await FileSystemEntity.type(destination.path, followLinks: false);
    final current = await destination.stat();
    if ((_original == null && currentType != FileSystemEntityType.notFound) ||
        (_original != null && (currentType != FileSystemEntityType.file ||
            current.size != _original.size || current.modified != _original.modified ||
            current.changed != _original.changed))) {
      throw const FileSystemException('The destination changed during export. Choose another filename and retry.');
    }
    if (_original != null) {
    ...
    try {
      ...
      if (_hasBackup) {
        try {
          ...
          throw FileSystemException('Could not finalize export. Your previous video is preserved at $_backupPath.');
        }
      }
      rethrow;
    }
