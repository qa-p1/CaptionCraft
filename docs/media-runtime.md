...
Instagram extraction uses the maintained [yt-dlp package](https://github.com/yt-dlp/yt-dlp). YouTube uses youtube_explode_dart with refreshed Android VR/Safari manifests, then falls back to yt-dlp if extraction or transfer fails. Website parsing belongs to those packages.
...
flutter test test/instagram_download_service_test.dart test/youtube_download_service_test.dart
```
